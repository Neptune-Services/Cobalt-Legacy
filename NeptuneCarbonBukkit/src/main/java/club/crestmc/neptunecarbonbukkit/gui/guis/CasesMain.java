        ...
        if(allCases.filter(new Document("type", "mute").append("uuid", target.getUuid().toString())).first() != null) {
            ...
            muteButton.setAction(() -> {
                ...
                new BukkitRunnable() {
                    @Override
                    public void run() {
                        CasesDetailed detailedPunishmentHistoryGUI = new CasesDetailed(player, 27, coloredName + "&2's Mutes");
        ...
        if(allCases.filter(new Document("type", "kick").append("uuid", target.getUuid().toString())).first() != null) {
            ...
            kickButton.setAction(() -> {
                ...
                new BukkitRunnable() {
                    @Override
                    public void run() {
                        CasesDetailed detailedPunishmentHistoryGUI = new CasesDetailed(player, 27, coloredName + "&2's Kicks");
        ...
        if(allCases.filter(new Document("type", "ban").append("uuid", target.getUuid().toString())).first() != null) {
            ...
            banButton.setAction(() -> {
                ...
                new BukkitRunnable() {
                    @Override
                    public void run() {
                        CasesDetailed detailedPunishmentHistoryGUI = new CasesDetailed(player, 27, coloredName + "&2's Bans");
        ...
        if(allCases.filter(new Document("type", "blacklist").append("uuid", target.getUuid().toString())).first() != null) {
            ...
            blacklistButton.setAction(() -> {
                ...
                new BukkitRunnable() {
                    @Override
                    public void run() {
                        CasesDetailed detailedPunishmentHistoryGUI = new CasesDetailed(player, 27, coloredName + "&2's Blacklists");
